#!/usr/bin/env python3
"""Build and audit retrieval-oriented Markdown documents from arXiv LaTeX."""
...
from .cases import cleanup as cleanup_case
from .cases import figures as figure_case
from .cases import source_selection
from .cases import tabbing as tabbing_case
from .cases import tables as table_case

THEOREM_NAMES = (
    "theorem", "lemma", "proposition", "corollary", "definition", "claim",
    "conjecture", "remark", "example", "proof", "tef", "prop", "lem", "teo", "defin",
    "nemma",
)
IN_SCOPE_FIGURE_CONTENT = re.compile(
    r"\\begin\{(?:verbatim|alltt|lstlisting|tabbing|tabular|array|equation|align|eqnarray|algorithm|"
    r"algorithmic|program|enumerate|itemize|description|minipage)\*?\}"
    r"|\\(?:parbox|shortex|eenumsentence)\b|\\\[|\$\$",
    re.IGNORECASE,
)
DISPLAY_MATH_SOURCE = re.compile(
    r"\$\$.*?\$\$|\\\[.*?\\\]|\\begin\{(?:equation|align|eqnarray|gather|multline|displaymath|array|cases)\*?\}",
    re.DOTALL,
)
...
SUSPICIOUS_FLATTENING = re.compile(
    r"\b(?:Let|Where|where|function|solution)[A-Z_a-z]\w*be\b|(?:^|\s)_{2,}(?:\s|$)"
    r"|BACKSLASHCHAR|LINEBREAKCHAR|ZZFB\d{6}ZZ|\d{6}ZZ|⟬\d{6}⟭"
)
...
AMSTEX_BIBLIOGRAPHY_ENVIRONMENT = re.compile(
    r"\\Refs\b.*?\\endRefs\b", re.DOTALL | re.IGNORECASE
)
...
MARKDOWN_BIBLIOGRAPHY_HEADING = re.compile(
...
LITERAL_CODE_ENVS = {"verbatim", "verbatim*", "lstlisting", "listing", "acmlisting", "verbcode", "ce"}
...
BASE_FORMAL_ENVS = {
    ...
    "figure", "figure*", "algorithm", "algorithm*", "algorithmic", "program", "tabbing",
    "picture", "pspicture", "avm",
    "description", "description*",
    "ex", "exo", "subex",
    "equation", "equation*", "align", "align*", "aligned", "aligned*", "eqnarray", "eqnarray*",
    "array", "matrix", "pmatrix", "bmatrix", "vmatrix", "cases", "gather", "gather*", "split",
    "multline", "multline*", "displaymath", "math",
    FORMAL_RAW_TABLE_ENV,
} | CODE_ENVS | TEXT_EXAMPLE_ENVS
...
OVERFULL_DRAFT_CONDITIONAL = re.compile(
    r"\\ifdim\s*\\overfullrule\b(?:(?!\\if).)*?\\fi", re.DOTALL
)
...
MATH_ENVS = {
    ...
    "vmatrix", "cases", "gather", "gather*", "split", "multline",
    "multline*", "displaymath", "math",
}
STRUCTURAL_MACRO_NAMES = {
    "begin", "end", "item", "label", "ref", "cite", "section", "subsection",
    "subsubsection", "chapter", "part", "paragraph", "abstract", "title", "author",
}
...
def retrieval_math_identifier_spacing(text: str) -> str:
    """Do not run legacy math-spacing heuristics over retrieval prose."""
    return text


# The archived converter's broad patterns corrupt ordinary words such as
# "ask" and "styles". Override them only for this active retrieval process.
...
def count_environments(text: str, names: tuple[str, ...]) -> int:
    ...
    return len(re.findall(rf"\\begin\s*\{{(?:{alternatives})\*?\}}", text, re.IGNORECASE))


def count_figures_with_in_scope_content(source: str) -> int:
    count = 0
    for match in legacy.RE_FIGURE_ENV.finditer(source):
        if IN_SCOPE_FIGURE_CONTENT.search(match.group(1)):
            count += 1
    return count


def source_inventory(source: str) -> dict[str, int]:
    without_figures = remove_environment_spans(source, {"figure", "figure*"})
    table_scope = remove_environment_spans(without_figures, MATH_ENVS)
    return {
        "sections": len(legacy.source_section_titles(without_figures)),
        "display_math": len(DISPLAY_MATH_SOURCE.findall(without_figures)),
        "tables": count_environments(table_scope, ("table", "tabular", "longtable")),
        "theorems": count_environments(without_figures, THEOREM_NAMES),
        "figures": count_environments(source, ("figure",)),
        "figures_with_in_scope_content": count_figures_with_in_scope_content(source),
        "algorithms": count_environments(
            without_figures, ("algorithm", "algorithmic", "program", "tabbing")
        ),
        "footnotes": len(re.findall(r"\\footnote\s*\{", without_figures)),
        "diagram_commands": len(re.findall(
            r"\\(?:xymatrix|pstree|begin\{(?:picture|pspicture|avm)\})", without_figures
        )),
    }
...
def markdown_inventory(markdown: str, source: str = "") -> dict[str, int]:
    labels = set(name.title() for name in THEOREM_NAMES)
    labels.update(theorem_environment_names(source).values())
    theorem_pattern = "|".join(re.escape(name) for name in labels)
    raw_tables = count_environments(markdown, ("table", "tabular", "longtable"))
    raw_theorems = count_environments(markdown, THEOREM_NAMES)
    raw_figures = count_environments(markdown, ("figure",))
    return {
        "headings": len(re.findall(r"(?m)^#{1,6}\s+\S", markdown)),
        "display_math": len(DISPLAY_MATH_SOURCE.findall(markdown)),
        "table_blocks": len(MARKDOWN_TABLE.findall(markdown)) + len(PANDOC_GRID_TABLE.findall(markdown)) + raw_tables,
        "theorem_labels": len(re.findall(
            rf"(?mi)^(?:#+\s*)?(?:\*\*)?(?:{theorem_pattern})\b", markdown
        ))
        + raw_theorems,
        "figure_evidence": len(re.findall(r"(?mi)^Figure\b|\[Figure omitted", markdown)) + raw_figures,
        "code_fences": markdown.count("```") // 2,
        "footnotes": len(re.findall(r"(?m)^\[\^[^]]+\]:|\(Footnote:", markdown)),
        "raw_latex_fences": len(re.findall(r"```latex\b", markdown)),
    }


def risk_flags(source: dict[str, int], output: dict[str, int], markdown: str, warnings: list[str]) -> list[str]:
    flags = []
    if source["display_math"] and not output["display_math"]:
        flags.append("missing_display_math")
    if source["tables"] and not output["table_blocks"] and not output["code_fences"]:
        flags.append("missing_table_evidence")
    if source["theorems"] >= 2 and output["theorem_labels"] < source["theorems"] // 2:
        flags.append("theorem_structure_loss")
    if source["footnotes"] and not output["footnotes"]:
        flags.append("footnote_loss_risk")
    if SUSPICIOUS_FLATTENING.search(markdown):
        flags.append("suspicious_math_flattening")
    if "used_plain_fallback_for_completeness" in warnings:
        flags.append("plain_fallback_risk")
    return flags


def retrieval_status(flags: list[str]) -> str:
    major = {"missing_display_math", "missing_table_evidence", "theorem_structure_loss"}
    if major.intersection(flags):
        return "high_risk"
    return "review" if flags else "clean_candidate"


def theorem_environment_names(source: str) -> dict[str, str]:
    aliases = {"tef": "Theorem", "teo": "Theorem", "prop": "Proposition",
               "lem": "Lemma", "nemma": "Lemma", "defin": "Definition"}
    names = {name: aliases.get(name, name.title()) for name in THEOREM_NAMES}
    names.update({env.lower(): title for _, _, env, title
                  in legacy.collect_newtheorem_definitions(source)})
    return names
...
def collect_commented_newcommand_macros(text: str) -> list[tuple[int, int, str, int, str]]:
    ...
    for match in pattern.finditer(text):
        ...
        if close >= 0:
            macros.append((match.start(), close + 1, match.group(1),
                           int(match.group(2) or 0), text[match.end():close]))
    return macros
...
def collect_spaced_command_macros(text: str) -> list[tuple[int, int, str, int, str]]:
    ...
    for match in pattern.finditer(text):
        close = legacy.find_matching_brace(text, match.end() - 1)
        if close >= 0:
            macros.append((match.start(), close + 1, match.group(1), int(match.group(2) or 0),
...
def unscoped_retrieval_macros(text: str) -> list[tuple[int, int, str, int, str]]:
    ...
    return [macro for macro in retrieval_macros(text)
...
def expand_retrieval_macros(text: str, definitions: str | None = None) -> str:
    ...
    macros = retrieval_macros(definitions) if definitions is not None else unscoped_retrieval_macros(text)
    ...
    for _ in range(3):
        dependencies = set().union(*(set(re.findall(r"\\[A-Za-z]+", body))
                                     for _, _, body in latest_by_name.values())) if latest_by_name else set()
        for _, _, name, nargs, body in macros:
            if name in dependencies:
                latest_by_name[name] = (name, nargs, body)
    safe = [macro for macro in latest_by_name.values()
            if macro[0] not in optional_names and safe_retrieval_macro(*macro)][:1000]
    for _ in range(2):
...
def substitute_macro_arguments(body: str, arguments: list[str]) -> str:
    for index, argument in enumerate(arguments, 1):
        marker = f"#{index}"
        body = re.sub(rf"(\\[A-Za-z]+){re.escape(marker)}", 
                      lambda match: f"{match.group(1)}{{{argument}}}", body)
...
def choice_macro_replacement(body: str, argument: str) -> str | None:
    ...
    if not match:
        return None
    branch = match.group(1) if argument == "*" else match.group(2) if argument == "-" else match.group(3)
...
def source_abstract_parts(source: str) -> tuple[int, int, str] | None:
    ...
    match = re.search(r"\\abstract\b(.*?)(?=\\(?:section|chapter|head)\*?\s*\{)",
                      source[search_start:], re.DOTALL | re.IGNORECASE)
    if match:
...
def strip_noncontent_source(text: str) -> str:
    ...
    return re.sub(
        ...
        lambda match: rf"\section{{{match.group(1)}}}", text, flags=re.IGNORECASE,
    )
...
def strip_bibliography(text: str) -> str:
    ...
    headings = [match for pattern in (BIBLIOGRAPHY_HEADING, MANUAL_BIBLIOGRAPHY_HEADING)
...
def preserve_description_labels(text: str) -> str:
    return re.sub(
        r"\\item\s*\[([^\]\n]+)\]",
        lambda match: f"\n- **{legacy.clean_metadata_text(match.group(1)).strip()}:** ", text,
    )
...
def trailing_scientific_appendix(tail: str) -> str:
    marker = re.search(
        r"(?im)^[ \t]*\\(?:appendix\b|(?:section|chapter)\*?\s*\{)", tail
    )
    if not marker:
        return ""
    ...
    heading = heading or re.match(r"\\(?:section|chapter)\*?\s*\{", appendix.lstrip(), re.IGNORECASE)
...
def strip_custom_figure_calls(text: str) -> str:
    ...
    graphics = re.compile(r"\\(?:epsfig|psfig|epsfbox|epsffile|includegraphics)\*?\b", re.IGNORECASE)
...
def strip_declared_graphic_calls(text: str, definitions: str) -> str:
    ...
    wrappers = {(name, nargs) for _, _, name, nargs, body in retrieval_macros(definitions)
...
def protect_inline_code_runs(text: str) -> str:
    ...
    while match := pattern.search(text, cursor):
        ...
        while True:
            separator = re.match(r"(?:\s|\{\\(?:small|footnotesize|scriptsize|tiny)\s+\\par\})*", text[end:])
...
def strip_graphic_wrappers(text: str) -> str:
    ...
    for match in reversed(list(pattern.finditer(text))):
        parsed = read_macro_args(text, match.end(), 3)
        if parsed and re.search(r"\\(?:includegraphics|epsfig|psfig)\b", parsed[0][2], re.IGNORECASE):
            text = text[:match.start()] + text[parsed[1]:]
    return text
...
def strip_figure_caption_sections(text: str) -> str:
    heading = re.compile(
        ...
        r"figure(?:s|\s+captions?)?\s*\}", re.IGNORECASE,
    )
...
def strip_table_of_contents(text: str) -> str:
    heading = re.compile(r"\\(?:section|subsection)\*?\s*\{\s*Table\s+of\s+Contents\s*\}", re.IGNORECASE)
...
def strip_page_layout_metadata(text: str) -> str:
    names = re.compile(r"\\(pagestyle|thispagestyle|markboth|markright|markleft|fancyhead|fancyfoot)\b")
...
def strip_layout_outside_formal_content(text: str) -> str:
    ...
    targets = MATH_ENVS | {"table", "table*", "tabular", "tabular*", "longtable", "tabbing", "description", "description*"}
...
def clean_evidence_label(value: str) -> str:
    value = legacy.clean_metadata_text(value)
...
def render_table_block(block: str) -> str:
    ...
    if tabbing_spans:
        ...
        rendered = [render_tabbing_block(block[start:end]).strip()
                    for start, end, _ in tabbing_spans]
    ...
    if pandoc_table:
        return f"\n\n{pandoc_table}\n\n"
    caption = clean_table_caption(block)
    fragments = [block[start:end] for start, end, _ in environment_spans(
        block, {"tabular", "tabular*", "longtable"}
    )]
...
def description_table_markdown(block: str) -> str:
    ...
    cleaned = [re.sub(r"\s*\d+(?:cm|mm|pt)\s+\d+(?:cm|mm|pt)\s*$", "", item)
               for item in cleaned]
...
def merge_table_continuation_rows(rows: list[list[str]]) -> list[list[str]]:
    ...
    for row in rows:
        ...
        continuation = (
            len(row) >= 3 and len(previous) == len(row)
            and bool(row[0]) and not row[1] and row[2].lstrip().startswith("(")
            and bool(previous[0]) and bool(previous[1]) and bool(previous[2])
            and row[0][:1].islower() and previous[0][:1].islower()
        )
        if not continuation:
            merged.append(row)
            continue
...
def strip_table_rules(text: str) -> str:
    text = re.sub(r"\\(?:hline|toprule|midrule|bottomrule|tableline)\b", "\n", text, flags=re.IGNORECASE)
...
def clean_table_cell(cell: str) -> str:
    ...
    cell = re.sub(r"\\(?:centering|raggedright|raggedleft|small|footnotesize|scriptsize)\b", " ", cell)
...
def table_has_fragile_math(block: str) -> bool:
    return bool(re.search(r"\$|\\\[|\\begin\{(?:array|equation|align|cases)\}", block))
...
def graphics_names(block: str) -> list[str]:
    names = []
    for match in legacy.RE_GRAPHICS_COMMAND.finditer(block):
        name = legacy.graphics_filename(match.group(1))
        if name and name not in names:
            names.append(name)
    return names


def figure_has_in_scope_content(block: str) -> bool:
    if IN_SCOPE_FIGURE_CONTENT.search(block):
        return True
    if caption_has_fragile_math(raw_latex_caption(block)):
        return True
    if graphics_names(block):
        return False
    if re.search(r"\\(?:xymatrix|pstree)\b|\\begin\{(?:picture|pspicture)\}", block):
        return False
    body = legacy.remove_latex_caption_commands(block)
    body = legacy.RE_LABEL_COMMAND.sub("", body)
    body = re.sub(r"\\(?:begin|end)\{figure\*?\}", "", body)
    words = re.findall(r"[A-Za-z]{3,}|\\[A-Za-z]+|[$_^]", body)
    return len(words) >= 5
...
def caption_has_fragile_math(caption: str) -> bool:
    return bool(re.search(r"\$|\\[A-Za-z]+|[_^]", caption))


def raw_latex_caption(block: str) -> str:
    match = legacy.RE_CAPTION_COMMAND.search(block)
    if not match:
        return ""
    close = legacy.find_matching_brace(block, match.end() - 1)
    return block[match.end():close] if close >= 0 else ""
...
def render_formal_body(body: str, theorem_envs: dict[str, str]) -> str:
    ...
    formal_envs |= {"tabbing", "description", "description*", "ex", "exo", "subex"} | set(theorem_envs)
...
def render_description_block(block: str, theorem_envs: dict[str, str]) -> str:
    ...
    spans = [span for span in environment_spans(body, {"description", "description*"})
...
def description_body(block: str) -> str:
    ...
    return block[start.end():end.start()] if start and end and end.start() >= start.end() else block
...
def render_description_item(label: str, content: str, children: dict[str, str]) -> str:
    ...
    rendered = prefix if not content else f"{prefix} {content}" if "\n" not in content else f"{prefix}\n{indent_markdown(content, 2)}"
...
def clean_description_content(text: str) -> str:
    empty_group = r"~?\s*\{\s*(?:\\(?:small|footnotesize|scriptsize|tiny|normalsize)\s*)?(?:\\par\s*)?\}"
    previous = None
    while text != previous:
        previous = text
...
def replace_spans(text: str, spans: list[tuple[int, int, str]], theorem_envs: dict[str, str],
                  blocks: dict[str, str] | None = None) -> tuple[str, dict[str, str]]:
...
def protect_formal_macro_calls(text: str, source: str, blocks: dict[str, str]) -> str:
    signatures = formal_macro_signatures(source)
    for name, nargs in signatures.items():
        pattern = re.compile(re.escape(name) + r"(?![A-Za-z])")
        for match in reversed(list(pattern.finditer(text))):
            parsed = read_macro_args(text, match.end(), nargs)
            if not parsed:
            ...
            _, end = parsed
            token = formal_token(len(blocks))
            blocks[token] = latex_fence("### Formal Rule", text[match.start():end])
...
def protect_remaining_macro_uses(text: str, source: str, blocks: dict[str, str]) -> str:
    ...
    for name, (nargs, body) in signatures.items():
        ...
        for match in reversed(list(pattern.finditer(text))[:300]):
            ...
            blocks[token] = f"`{evidence[:500]}`" if "\n" not in evidence else latex_fence("### Formal Source", evidence)
...
def remaining_macro_signatures(source: str, text: str) -> dict[str, tuple[int, str]]:
    ...
    return {name: (nargs, body) for name, (nargs, body) in definitions.items()
            if name not in optional_names and name in used
            and semantic.search(body) and len(body) <= 1000}


def formal_macro_signatures(source: str) -> dict[str, int]:
    latest = {}
    formal = re.compile(r"\\(?:begin\{(?:equation|align|array)|longrightarrow|rightarrow|frac)\b|[$&_^]")
    for _, _, name, nargs, body in retrieval_macros(source):
        latest[name] = (nargs, body)
    optional_names = optional_argument_macro_names(source)
    return {name: nargs for name, (nargs, body) in latest.items()
            if name not in optional_names and nargs and (nargs > 3 or formal.search(body))}
...
def decode_encoded_code(value: str) -> str:
    replacements = {
        "LINEBREAKCHAR": "\n", "BACKSLASHCHAR": "\\",
        "PERCENTCHAR": "%", "LEFTBRACECHAR": "{", "RIGHTBRACECHAR": "}",
    }
...
def protect_fragile_dollar_displays(text: str, blocks: dict[str, str]) -> str:
    spans = [match.span() for match in FRAGILE_DOLLAR_DISPLAY.finditer(text)
...
def section_coverage(source: str, markdown: str) -> float:
    titles = legacy.source_section_titles(source)
    if not titles:
        return 1.0
    hits, _ = legacy.section_title_hits(titles, markdown)
    return hits / len(titles)


def retained_formal_tokens(source: str, markdown: str) -> bool:
    return set(FORMAL_TOKEN_RE.findall(source)).issubset(FORMAL_TOKEN_RE.findall(markdown))
...
def source_preserving_prose(source: str) -> str:
    ...
    text = re.sub(r"\\begin\{(?:mylist|list)\}(?:\s*\{[^{}]*\}){0,2}", "\n", text, flags=re.IGNORECASE)
...
def strip_direct_layout_commands(text: str) -> str:
    ...
    text = re.sub(
        r"\\begin\{(?:center|flushleft|flushright|quote|quotation)\}", "\n", text
    )
    text = re.sub(
        r"\\end\{(?:center|flushleft|flushright|quote|quotation)\}", "\n", text
    )
    ...
    text = re.sub(r"\\newline\b", " ", text)
    text = re.sub(r"\\(?:newpage|clearpage|pagebreak|eject|vfill|vfil|noindent|indent|centering|raggedright|raggedleft)\b", " ", text)
    ...
    text = re.sub(r"\\rule(?:\s*\[[^\]]*\])?\s*\{[^{}]*\}\s*\{[^{}]*\}|\\hrulefill\b|\\hrule\b", " ", text)
    text = re.sub(r"\\\\(?:\s*\[[^\]\n]*\])?", "\n", text)
    text = re.sub(r"\\(?:setcounter|addtocounter|setlength)\s*\{[^{}]*\}\s*\{[^{}]*\}", " ", text)
    text = re.sub(r"\\pagenumbering\s*\{[^{}]*\}", " ", text)
    text = re.sub(r"\\(?:figrule|border|cbstart|cbend|qtreecenterfalse|qtreecentertrue|strich|unskip|kill|qed)\b", " ", text)
    ...
    text = re.sub(r"\\(?:small|footnotesize|scriptsize|tiny|normalsize|large|Large|LARGE|huge|Huge)\b", " ", text)
    text = re.sub(r"\\(?:protect|leavevmode|relax|sloppy|fussy|draft|findemo|widetext|hfill|break)\b", " ", text)
    text = re.sub(r"(?m)^\s*\\(?:maketitle|tableofcontents|listoffigures|listoftables)\s*$", "", text)
...
def demote_false_headings(markdown: str, source: str) -> str:
    titles = {legacy.normalized_title_text(strip_heading_label(title)) for title in legacy.source_section_titles(source)}
    output = []
...
def local_macro_map(source: str) -> dict[str, tuple[str, str]]:
    definitions = {}
    for pattern in (legacy.RE_NEWCOMMAND, legacy.RE_DEF, legacy.RE_DEFINE):
        for start, end, name, _, body in legacy.collect_macros(pattern, source):
            if "@" not in name and name.lstrip("\\").lower() not in STRUCTURAL_MACRO_NAMES:
                definitions[name] = (source[start:end].strip(), body)
    return definitions


def source_body_for_macro_usage(source: str) -> str:
    text = legacy.strip_iffalse_blocks(source)
    text = legacy.RE_COMMENT_LINE.sub("", text)
    return legacy.strip_preamble(text)


def used_local_macro_definitions(source: str, markdown: str) -> list[str]:
    definitions = local_macro_map(source)
    used = set(re.findall(r"\\[A-Za-z]+", source_body_for_macro_usage(source) + markdown))
    queue = [name for name in definitions if name in used]
    selected = []
    seen = set()
    while queue and len(selected) < 200:
        ...
        if name in seen or name not in definitions:
            continue
        seen.add(name)
        raw, body = definitions[name]
        selected.append(raw)
        queue.extend(dep for dep in re.findall(r"\\[A-Za-z]+", body) if dep in definitions and dep not in seen)
    return selected


def append_macro_definitions(markdown: str, source: str) -> str:
    definitions = used_local_macro_definitions(source, markdown)
    if not definitions:
        return markdown
    appendix = "\n\n### Referenced Local Macro Definitions\n\n```latex\n"
...
def source_title_text(source: str) -> str:
    ...
    title = ""
    for match in reversed(list(SOURCE_TITLE.finditer(source))):
        close = legacy.find_matching_brace(source, match.end() - 1)
        candidate = source[match.end():close] if close >= 0 else ""
    ...
    title = legacy.drop_balanced_argument_commands(
        title, re.compile(r"\\(?:footnote|thanks)\s*\{")
    )
    title = legacy.drop_balanced_argument_commands(
    ...
    title = re.sub(r"\\(?:centerline|normalsize|small|large|Large|LARGE|huge|Huge)\b", "", title)
    title = re.sub(r"\\(?:rm|bf|it|sc|tt|large|Large|small)\b", "", title)
    title = title.replace(r"\\", " ").replace("{", "").replace("}", "")
    return legacy.clean_metadata_text(title)


def source_metadata_header(paper: dict, source: str) -> str:
    title = source_title_text(source) or legacy.clean_metadata_text(paper.get("title") or "Untitled")
    title = cleanup_case.clean_title(title) or legacy.clean_metadata_text(paper.get("title") or "Untitled")
    parts = [f"# {title}", ""]
...
def expand_abstract_macro(abstract: str, definitions: str) -> str:
    ...
    if not call:
        return abstract
    matches = [macro for macro in retrieval_macros(definitions)
...
def repair_split_decimal_emphasis(markdown: str) -> str:
    ...
    markdown = pattern.sub(lambda match: f"{match.group(1)}{match.group(2)}."
                           f"{match.group(3)}{match.group(4)}", markdown)
    split_value = re.compile(
        r"\*\*((?:\d+\.)+\d+\.\s+[^*\n]{3,300}?)\*\*\s*\n+\s*(\d{3})(?=[,;])"
    )
...
def repair_split_numbered_prose(markdown: str) -> str:
    ...
    markdown = re.sub(r"\b(equation\s*\(\d+)\*\*\s*\n+\s*(\d+\):)", r"\1.\2", markdown,
                      flags=re.IGNORECASE)
...
def clean_output_spacing(text: str) -> str:
    ...
    text = re.sub(r"\\hfill\s*\\rule(?:\s*\[[^\]]*\])?\s*\{[^{}]*\}\s*\{[^{}]*\}\s*\\vspace\s*\{[^{}]*\}", "", text)
    ...
    text = re.sub(r"^\s*(?:\{\s*)+\}?\s*$|^\s*\\textbf\s*\{\s*$", "", text,
                  flags=re.MULTILINE)
    text = re.sub(r"(?m)^\s*\\let\\[A-Za-z@]+.*$", "", text)
    text = re.sub(r"(?m)^\s*-?\s*sep\s+\S+\s*\{?bibdata\}?\s*$", "", text,
                  flags=re.IGNORECASE)
...
def strip_standalone_layout_braces(text: str) -> str:
    ...
    for line in text.splitlines():
        ...
        if not fenced:
            ...
            displayed |= bool(re.search(r"\\\[|\\begin\{(?:equation|align|gather|multline|displaymath)", line))
...
def normalize_final_structure(text: str) -> str:
    ...
    text = re.sub(r"\\begin\{(?:myitemize|mylist|list|slide\*?|examples)\}(?:\s*\{[^{}]*\}){0,2}",
                  "\n", text, flags=re.IGNORECASE)
    text = re.sub(r"\\end\{(?:myitemize|mylist|list|slide\*?|examples)\}", "\n", text,
                  flags=re.IGNORECASE)
...
def render_retrieval_document(paper: dict, source: str) -> tuple[str, str, list[str], str]:
    ...
    table_targets = {env for env in targets if env.startswith(("table", "tabular")) or env == "longtable"}
    ...
    combined = source_metadata_header(paper, source) + markdown
...
def retrieval_record(paper: dict, source: str, source_file: str, markdown: str, method: str, warnings: list[str]) -> dict:
    return {
        "id": paper.get("id"), "title": paper.get("title", ""), "abstract": paper.get("abstract", ""),
        "categories": paper.get("categories", ""), "authors": paper.get("authors", ""),
        "doi": paper.get("doi", ""), "update_date": paper.get("update_date", ""),
        "source_file": source_file, "source_format": "latex", "markdown": markdown,
        "conversion_method": method, "conversion_strategy": "retrieval_source_preserving",
        "full_text_converted": True, "warnings": warnings,
    }


def convert_paper(paper: dict, main_file: str | None = None) -> dict:
    raw_source = paper.get("latex") or ""
    selected = source_selection.select_paper_source(raw_source, main_file)
    if selected is None:
        raise ValueError(f"support-only source bundle for {paper.get('id')}")
    source, source_file, source_count = selected
    if legacy.detect_format(source) != "latex":
        raise ValueError(f"unsupported selected source for {paper.get('id')}")
    markdown, method, warnings, prepared = render_retrieval_document(paper, source)
    record = retrieval_record(paper, source, source_file, markdown, method, warnings)
    markdown = record["markdown"]
    before = source_inventory(prepared)
    after = markdown_inventory(markdown, source)
    flags = risk_flags(before, after, markdown, warnings)
    legacy_metrics = {
        "id": paper.get("id"), "title": paper.get("title", ""), "categories": paper.get("categories", ""),
        "method": method, "strategy": "retrieval_source_preserving", "source_format": "latex",
        "source_file": source_file, "source_file_count": source_count, "latex_len": len(raw_source),
        "selected_latex_len": len(source), "md_len": len(markdown),
        "ratio": round(len(markdown) / max(len(source), 1), 3), "warnings": warnings,
    }
    return {
        "record": record,
        "source": source,
        ...
        "markdown_inventory": after,
        "risk_flags": flags,
        "retrieval_status": retrieval_status(flags),
    }
