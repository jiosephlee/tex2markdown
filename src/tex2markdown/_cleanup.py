...
DROP_ARGUMENT_COMMAND = re.compile(
    r"\\(?:institute|dedicatory|affiliation|affil|copyrightnotice)\s*\{"
)
...
def strip_front_matter_residue(text: str) -> str:
    ...
    text = re.sub(
        r"\\begin\{bottomstuff\}.*?\\end\{bottomstuff\}", "", text, flags=re.DOTALL | re.IGNORECASE
    )
    text = re.sub(
        r"\\twocolumn\s*\[\s*\\hsize\\textwidth\\columnwidth\\hsize"
        r"\\csname@twocolumnfalse%?\s*\\endcsname",
        "",
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(
        r"\\begin\{frontmatter\}.*?\\end\{frontmatter\}", "", text, flags=re.DOTALL | re.IGNORECASE
    )
    text = re.sub(
        r"\\twocolumn\[.*?(?=\\(?:sloppy|narrowtext)\b)", "", text, flags=re.DOTALL | re.IGNORECASE
    )
    text = legacy.drop_balanced_argument_commands(text, DROP_ARGUMENT_COMMAND)
    text = DROP_BARE_COMMAND.sub(" ", text)
    text = re.sub(
        r"\\(?:sloppy|narrowtext|twocolumn|onecolumn|columnwidth|textwidth|hsize)\b", " ", text
    )
    ...
    text = legacy.drop_balanced_argument_commands(
        text,
        re.compile(
            r"\\(?:journame|volnumber|issuenumber|issuemonth|volyear|received|revised|"
            r"authorrunninghead|titlerunninghead|shortauthor|shorttitle)\s*\{"
        ),
    )
    text = re.sub(r"(?m)^\s*\[(?:h|t|b|p|!)+\]\s*$", "", text, flags=re.IGNORECASE)
    text = re.sub(
        r"\\begin\s*\{(?:titlepage|center|flushleft|flushright)\}|"
        r"\\end\s*\{(?:titlepage|center|flushleft|flushright)\}",
        " ",
        text,
        flags=re.IGNORECASE,
    )
...
def strip_publication_front_matter(text: str) -> str:
    text = re.sub(r"(?is)\bPrevious Issues of [^:\n]{1,80}:.*?(?=\\begin\{titlepage\})", "", text)
    text = re.sub(
    ...
    pattern = re.compile(r"\\begin\{titlepage\}(.*?)\\end\{titlepage\}", re.DOTALL | re.IGNORECASE)

    def replacement(match: re.Match) -> str:
        body = match.group(1)
        scientific = re.search(
            r"\\(?:section|chapter|begin\{(?:abstract|equation|tabular))\b", body
        )
...
def strip_layout_declarations(text: str) -> str:
    ...
    text = re.sub(r"\\(?:setlength|addtolength)\s*\{\\[A-Za-z@]+\}\s*\{[^{}]*\}", " ", text)
...
def strip_declaration_preamble(text: str) -> str:
    ...
    declarations = len(
        re.findall(
            r"\\(?:newcommand|renewcommand|newenvironment|renewenvironment|newtheorem|let)\b",
            prefix,
        )
    )
...
def normalize_manual_sections(text: str) -> str:
    text = re.sub(
        ...
        lambda match: rf"\section{{{' '.join(match.group(1).split())}}}",
        text,
    ...
    text = re.sub(
        ...
        lambda match: rf"\subsubsection*{{{match.group(1).strip()}}}",
        text,
        flags=re.IGNORECASE,
    )
    ...
    text = re.sub(
        ...
        lambda match: rf"\section*{{{match.group(1).strip()}}}",
        text,
    )
    return re.sub(
