...
def _clean_formal_table_shell(source: str) -> str:
    ...
    item_macros = re.findall(r"\\def\s*(\\[A-Za-z]+)\s*\{\s*\\item(?:\s*\[[^\]]*\])?\s*\}", source)
    source = re.sub(r"\\def\s*\\[A-Za-z]+\s*\{\s*\\item(?:\s*\[[^\]]*\])?\s*\}", "", source)
    for name in item_macros:
        source = re.sub(re.escape(name) + r"(?![A-Za-z])\s*", "\n- ", source)
    source = re.sub(r"\\fbox\s*\{", "", source)
    source = re.sub(
        r"\\(?:vspace|hspace)\*?\s*\{[^{}]*\}|\\(?:bigskip|medskip|smallskip)\b", " ", source
    )
    source = re.sub(r"\\(?:centering|noindent)\b", " ", source)
    source = re.sub(r"\{\\bf\s+(Axioms?|Rules?)\}", r"### \1", source, flags=re.IGNORECASE)
...
def _table_spans(text: str) -> list[tuple[int, int]]:
    ...
    for match in token.finditer(text):
        if match.group(1).lower() == "begin":
            ...
            continue
        index = next(
            (i for i in range(len(stack) - 1, -1, -1) if stack[i][0] == match.group(2).lower()),
            None,
        )
...
def should_preserve_source(block: str) -> bool:
    ...
    fragile_layout = bool(
        re.search(
            r"\\(?:shortstack|fbox|parbox|char\d+|bordermatrix)\b|\\\|",
            block,
            re.IGNORECASE,
        )
    )
...
def source_table_content(block: str) -> str:
    ...
    block = re.sub(
        r"\\begin\{table\*?\}(?:\[[^\]]*\])?|\\end\{table\*?\}", "", block, flags=re.IGNORECASE
    )
